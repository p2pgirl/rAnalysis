...
```{r echo=FALSE, message=FALSE, warning=FALSE, unique_values}
...
```{r echo=FALSE, message=FALSE, warning=FALSE, table_values}
...
```{r echo=FALSE, message=FALSE, warning=FALSE,  column_change}
...
p1 <- ggplot (data = coffeedata, aes(x = Expiration)) +
...
There is some overplotting and I'd like to be able to see the data better, so I produced a second plot where I limited my y axis to have the most common data which helps eliminate some of the ouliers as well.
...
ggplot(aes(x = Country.of.Origin, y = Total.Cup.Points), data = coffeedata) +
  geom_point(alpha = .25, color = 'blue', shape = 4) +
  ylim(70, 90) +
  theme(axis.text.x = element_text(angle = 90)) +
  xlab("Country of Origin") +
  ggtitle("Total Cup Points by Country")
...
ggplot(aes(x = Moisture, y = Total.Cup.Points), data = coffeedata) +
  geom_point(alpha = 1/10, color = 'blue', shape = 2, position = position_jitter(h=0)) +
  xlim(0, .2) +
  ylim(60, 90) +
  xlab("Moisture Measurement of Beans") +
  ylab("Total Cup Points") +
...
```{r echo=FALSE, message=FALSE, warning=FALSE, country_processing}
...
```{r echo=FALSE, message=FALSE, warning=FALSE, Corr_Matrix}
#get columns for just testing data
coffeemat <- coffeedata[, c(9:17)]
head(coffeemat)
```
...
The **Average Total Cup Points by Coffee Variety** give a quick look as to what types of variety of beans to look for when finding good coffee.  Higher on the scale for the variety shows the higher quality coffee.
